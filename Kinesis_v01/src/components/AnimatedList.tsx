import { useRef, useState, useEffect, ReactNode, MouseEventHandler, UIEvent } from 'react';
...
const AnimatedItem: React.FC<AnimatedItemProps> = ({ children, delay = 0, index, onMouseEnter, onClick }) => {
  ...
  return (
    <motion.div
      ...
      animate={inView ? { scale: 1, opacity: 1 } : { scale: 0.9, opacity: 1 }}
...
const AnimatedList: React.FC<AnimatedListProps> = ({
  ...
  return (
    <div className={`relative w-[450px] ${className}`}>
      <div
        ref={listRef}
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div className={`p-6 bg-[#111] rounded-lg ${selectedIndex === index ? 'bg-[#222]' : ''} ${itemClassName}`}>
              <p className="text-white m-2">{item}</p>
            </div>
          </AnimatedItem>
        ))}
      </div>
